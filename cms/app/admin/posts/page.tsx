  ...
  return (
    <div className="px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-[22px] font-semibold tracking-tight" style={{ color: 'var(--text)' }}>Blog Posts</h1>
          <p className="text-[13px] mt-0.5" style={{ color: 'var(--muted)' }}>{posts.length} total · {posts.filter(p => p.published).length} published</p>
        </div>
      ...
      <div className="rounded-[10px] overflow-hidden border" style={{ borderColor: 'var(--border)' }}>
        {posts.length === 0 ? (
          <div className="py-16 text-center" style={{ color: 'var(--muted)' }}>
            ...
            <Link href="/admin/posts/new" className="text-[13px] mt-2 inline-block" style={{ color: 'var(--teal)' }}>Create your first post →</Link>
          </div>
        ) : (
            ...
            <tbody>
              {posts.map((post, i) => (
                <tr key={post.id} style={{ borderTop: i === 0 ? 'none' : '1px solid var(--border)' }}>
                  <td className="px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--text)' }}>{post.title}</td>
                  <td className="px-5 py-4 text-[13px] font-mono" style={{ color: 'var(--muted)' }}>/{post.slug}</td>
                  ...
                  <td className="px-5 py-4 text-[13px]" style={{ color: 'var(--muted)' }}>{new Date(post.updatedAt).toLocaleDateString()}</td>
